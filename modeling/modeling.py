...
from modeling.utils import deep_instantiate
from modeling.multimodal import MultimodalContent
import PIL
...
from modeling.import_utils import is_flash_attn_2_available, is_torch_fx_available


from modeling.logger import logger

from modeling.utils import pretty_format
...
class IMAGELLMRMSNorm(nn.Module):
...
ALL_LAYERNORM_LAYERS.append(IMAGELLMRMSNorm)
...
class IMAGELLMAttention(nn.Module):
...
class IMAGELLMFlashAttention2(IMAGELLMAttention):
...
class IMAGELLMDecoderLayer(nn.Module):
    def __init__(self, config: IMAGEConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = (
            IMAGELLMAttention(config=config)
            if not getattr(config, "_flash_attn_2_enabled", False)
            else IMAGELLMFlashAttention2(config=config)
        )
        self.mlp = IMAGELLMMLP(config)
        self.input_layernorm = IMAGELLMRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = IMAGELLMRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class IMAGELLMPreTrainedModel(PreTrainedModel, FSDPMixin):
    config_class = IMAGEConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["IMAGELLMDecoderLayer"]
    ...
    def _check_and_enable_flash_attn_2(
        ...
        if not is_flash_attn_2_available():
            ...
            if not is_flash_greater_than_2:
                raise ValueError(
                    f"You need flash_attn package version to be greater or equal than 2.1. Make sure to have that version installed."
                )
...
class IMAGELLMModel(IMAGELLMPreTrainedModel):

    def __init__(self, config: IMAGEConfig):
        super().__init__(config)
        if getattr(self.config, "_flash_attn_2_enabled", False):
            logger.info("IMAGELLMModel is using Flash Attention 2")

        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size

        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList([IMAGELLMDecoderLayer(config) for _ in range(config.num_hidden_layers)])
        self.norm = IMAGELLMRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        IMAGE_start_id = self.config.special_tokens2ids_dict["additional_special_tokens"][DEFAULT_DREAM_START_TOKEN]
        ...
        if images_dm is not None and inputs_embeds is not None:
            query_embedding = self.IMAGE_embedding(inputs_embeds.shape[0])
            new_input_embeds = []
                ...
                for _IMAGE_start_pos in IMAGE_start_pos:
                    cur_new_input_embeds = torch.cat(
                        [
                            ...
                            cur_query_embedding,
                            cur_new_input_embeds[_IMAGE_start_pos + self.IMAGE_embedding.embed_len + 1 :],
                        ],
                        dim=0,
                    ).contiguous()
    ...
    def prepare_IMAGE_queries_with_special_token(self, batch_size: int = 1):
        IMAGE_start_id = self.config.special_tokens2ids_dict["additional_special_tokens"][DEFAULT_DREAM_START_TOKEN]
        IMAGE_end_id = self.config.special_tokens2ids_dict["additional_special_tokens"][DEFAULT_DREAM_END_TOKEN]
        IMAGE_special_ids = [IMAGE_start_id, IMAGE_end_id]
        IMAGE_special_embeds = self.embed_tokens(torch.as_tensor([IMAGE_special_ids], device=self.device))
        IMAGE_queries = torch.cat(
            [IMAGE_special_embeds[..., :1, :], self.IMAGE_embedding(), IMAGE_special_embeds[..., 1:, :]], 1
        )
        return IMAGE_queries.repeat(batch_size, 1, 1).to(self.device)
...
class IMAGELLMForCausalMLM(IMAGELLMPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config: IMAGEConfig):
        super().__init__(config)
        self.model = IMAGELLMModel(config)
        self.vocab_size = config.vocab_size
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        model = super(IMAGELLMForCausalMLM, cls).from_pretrained(
    ...
    def forward(
        ...
        IMAGE_start_id = self.config.special_tokens2ids_dict["additional_special_tokens"][DEFAULT_DREAM_START_TOKEN]
        IMAGE_end_id = self.config.special_tokens2ids_dict["additional_special_tokens"][DEFAULT_DREAM_END_TOKEN]
        IMAGE_patch_id = self.config.special_tokens2ids_dict["additional_special_tokens"][DEFAULT_IMAGE_PATCH_TOKEN]
        ...
        if self.training and images_dm is not None:
            ...
            for _idx, cur_hidden_states in enumerate(hidden_states):
                ...
                for _IMAGE_start_pos in IMAGE_start_pos:
                    ...
                    encoder_hidden_states.append(
                        cur_hidden_states[_IMAGE_start_pos + 1 : _IMAGE_start_pos + 1 + self.model.IMAGE_embedding.embed_len]
                    )
                    if encoder_hidden_states[-1].shape[0] != self.model.IMAGE_embedding.embed_len:
                        logger.error(_idx, _IMAGE_start_pos, hidden_states.shape)
            ...
            if self.stable_diffusion_head.drop_prob is not None:
                u_outputs = self.model(
                    input_ids=torch.tensor(
                        [
                            ...
                            + [IMAGE_patch_id] * self.model.IMAGE_embedding.embed_len
                            + [IMAGE_end_id, eos_id]
                        ]
                    ).to(input_ids.device),
                    attention_mask=torch.ones(1, self.model.IMAGE_embedding.embed_len + 4, device=encoder_hidden_states.device),
                ...
                u_encoder_hidden_states = u_outputs[0][:, 2 : 2 + self.model.IMAGE_embedding.embed_len, :]
            ...
            vm_loss = self.stable_diffusion_head(images_dm, None, None, self.model.IMAGE_embedding())
    ...
    def get_prompt_embeds(
        ...
        IMAGE_query = self.model.prepare_IMAGE_queries_with_special_token(batch_size=batch_size)

        IMAGE_query_attention_mask = torch.ones(batch_size, IMAGE_query.shape[1], device=device)
        attention_mask = torch.cat([text_input_attention_mask, IMAGE_query_attention_mask], dim=1)

        out = self.forward(
            input_ids=None,
            attention_mask=attention_mask,
            past_key_values=text_out.past_key_values,
            inputs_embeds=IMAGE_query,
            use_cache=True,
    ...
    def stable_diffusion_pipeline(
        ...
        # NOTE: 4. IMAGELLMForCausalMLM only acts as a text encoder and leaves the rest to the stable diffusion head
