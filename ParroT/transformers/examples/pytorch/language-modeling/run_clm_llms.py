...
def setup_datasets(data_args, model_args, logger):
...
def setup_config(model_args, logger):
    config_kwargs = {
        "cache_dir": model_args.cache_dir,
    ...
    if model_args.config_name:
        ...
        config = IMAGELLMConfig.from_pretrained(model_args.model_name_or_path, **config_kwargs)
...
def setup_models(model_args, logger, config):
    tokenizer_kwargs = {
        "cache_dir": model_args.cache_dir,
    ...
    if model_args.model_name_or_path:
        ...
        model = IMAGELLMForCausalMLM.from_config(config)
    ...
    if "llama" in model_args.model_name_or_path:
        if "meta-llama-3-7b" in model_args.model_name_or_path:
            pass
        else:
...
def preprocess_data(training_args, data_args, tokenizer, raw_datasets, logger):
...
def train_fn(training_args, model_args, last_checkpoint, data_args, train_dataset, trainer, tokenizer):
    if training_args.do_train:
        checkpoint = None
...
def evaluate_fn(training_args, data_args, eval_dataset, trainer, logger):
    if training_args.do_eval:
        ...
        logger.info("No eval dataset.")

def push_to_hub_fn(model_args, data_args, training_args, trainer):
...
class SDArguments:
    # Model-related
    sd_model: str = field(
        default=None, metadata={"help": "Path to the Stable Diffusion Model"}
    )
    IMAGEllm_model: str = field(
        default=None, metadata={"help": "Path to the IMAGELLM Model"}
    )
    sim_model: str = field(
        default=None, metadata={"help": "Path to the similarity Model"}
    )
    # VSG-related
    detect_model: str = field(
        default=None, metadata={"help": "Path to the detection Model"}
    )
    detect_cfg: str = field(
        default=None, metadata={"help": "Path to the detection Config"}
    )
    topk: int = field(
        default=30, metadata={"help": "Number of VSG triplets to use"}
    )
    # Dataset-related: to be deprecated
    dataset: str = field(
        default=None, metadata={"help": "Path to the dataset"}
    )
    # Image-related
    img_storage_path: str = field(
        default=None, metadata={"help": "Path to the image storage"}
    )
    # Training Config for DDPOTrainer
    ddpo_num_epochs: int = field(
        default=1, metadata={"help": "Number of epochs to train for"}
    )
    ddpo_ckpt_dir: str = field(
        default=None, metadata={"help": "Path to the checkpoint directory"}
    )
    ddpo_logging_dir: str = field(
        default=None, metadata={"help": "Path to the log directory"}
    )
    ddpo_train_batch_size: int = field(
        default=16, metadata={"help": "Batch size for training"}
    )
    ddpo_sample_batch_size: int = field(
        default=16, metadata={"help": "Batch size for sampling"}
    )
    ddpo_train_learning_rate: float = field(
        default=5e-5,
    )
...
def main():
    ...
    # Init trainer
    trainer, train_dataset, eval_dataset = init_training(training_args, data_args, lm_datasets, logger, model, tokenizer)

    # Training
    train_fn(training_args, model_args, last_checkpoint, data_args, train_dataset, trainer, tokenizer)

    # Evaluation
    evaluate_fn(training_args, data_args, eval_dataset, trainer, logger)

    # Push to hub
    push_to_hub_fn(model_args, data_args, training_args, trainer)
