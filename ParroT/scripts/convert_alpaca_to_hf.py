...
def create_prompt_vicuna(path):
    list_data_dict = read_json(path)
    prompt_input = PROMPT_DICT["vicuna_prompt"]
...
def write_json(in_file, out_file, mode):
    if mode == "alpaca":
        prompts, sources = create_prompt(in_file)
    elif mode == "vicuna":
        prompts, sources = create_prompt_vicuna(in_file)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode','-m', type=str, required=True, help='mode: vicuna or alpaca')
    args = parser.parse_args()
    in_file = args.in_file
    out_file = args.out_file

    # Start
    write_json(in_file, out_file, args.mode)
